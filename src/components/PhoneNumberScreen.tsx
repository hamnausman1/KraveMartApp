...
import svgPaths from "../imports/svg-5to5z6flg7";
...
import { imgMain1 } from "../imports/svg-yjxht";
...
function Group() {
  return (
    <div className="absolute inset-[4.56%_46.4%_91.63%_5.07%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-18.893px_-36.834px] mask-size-[374.954px_811.834px]" data-name="Group" style={{ maskImage: `url('${imgMain1}')` }}>
...
function CommunicationCall24Px() {
  return (
    <div className="absolute left-[calc(50%-126px)] size-[24px] top-[calc(50%+0.5px)] translate-x-[-50%] translate-y-[-50%]" data-name="communication/call_24px">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24 24">
...
function Group1({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return (
    <div className="absolute h-[25px] left-[16px] overflow-clip top-[calc(50%-0.5px)] translate-y-[-50%] w-[276px]" data-name="Group">
      <CommunicationCall24Px />
      <div className="absolute bottom-0 flex flex-col font-['Poppins:Regular',sans-serif] justify-center leading-[0] left-[15.94%] not-italic opacity-[0.87] right-[-2.9%] text-[#37474f] text-[16px] top-[4%] tracking-[0.44px]">
        <input
          type="tel"
          value={value}
          onChange={(e) => onChange(e.target.value)}
          placeholder="03XX-XXXXXXX"
          className="leading-[normal] bg-transparent border-none outline-none w-full placeholder:text-[#37474f] placeholder:opacity-[0.87]"
        />
      </div>
    </div>
  );
...
function Group7Copy({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return (
    <div className="absolute inset-[70.57%_4.27%_23.03%_4.27%] overflow-clip" data-name="Group 7 Copy">
      <div className="absolute bg-[#f0f1f2] inset-0 rounded-[8px]" data-name="Rectangle" />
      <Group1 value={value} onChange={onChange} />
    </div>
  );
}

function FormElementsMobileInputFieldsActiveEmpty({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return (
    <div className="absolute contents inset-[70.57%_4.27%_23.03%_4.27%]" data-name="form elements/mobile/input fields/active/empty">
      <Group7Copy value={value} onChange={onChange} />
    </div>
  );
}
...
export default function PhoneNumberScreen({ onNext }: PhoneNumberScreenProps) {
  ...
  const handleSpeakText = () => {
    const pageText = `${t('enterMobileNumber')}. ${t('weWillVerify')}`;
    speak(pageText, language === 'urdu' ? 'urdu' : 'english');
  };

  return (
    <div className="bg-white relative size-full" data-name="Splash--Phone Number">
      <MaskGroup />
      <BarsStatusBarIPhoneLight />
      <BarsHomeIndicatorIPhoneLightPortrait />
      <FormElementsMobileInputFieldsActiveEmpty value={phoneNumber} onChange={handlePhoneNumberChange} />
      <UndrawPersonalizationTriu />

      <div className="absolute flex flex-col font-['Poppins:Medium',sans-serif] inset-[55.67%_25.87%_40.64%_4.27%] justify-center leading-[0] not-italic text-[#37474f] text-[20px] text-nowrap">
        <p className="leading-[normal] whitespace-pre">{t('enterMobileNumber')}</p>
      </div>
      <p className="absolute font-['Poppins:Regular',sans-serif] inset-[60.59%_4.27%_32.88%_4.27%] leading-[normal] not-italic text-[14px] text-[rgba(55,71,79,0.72)]">{t('weWillVerify')}</p>
      
      {/* Error Message */}
      {error && (
        <p className="absolute left-[4.27%] right-[4.27%] top-[78%] font-['Poppins:Regular',sans-serif] text-[12px] text-red-500 text-center">
          {error}
        </p>
      )}

      <ButtonsMobileSolidTextSymbolTextSymbol onClick={handleNext} disabled={!phoneNumber.trim()} />
