...
import svgPaths from "../imports/svg-imn0karg43";
...
import { imgMain1 } from "../imports/svg-c8p7f";
...
function Group({ onClick }: { onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
      className="absolute block cursor-pointer inset-[5.67%_47.2%_90.52%_4.27%] mask-alpha mask-intersect mask-no-clip mask-no-repeat mask-position-[-15.893px_-45.834px] mask-size-[374.954px_811.834px] bg-transparent border-none" 
      data-name="Group" 
      style={{ maskImage: `url('${imgMain1}')` }}
    >
...
function MaskGroup({ onBack }: { onBack: () => void }) {
  return (
...
function OTPBox({ value, index, onChange, onKeyDown, inputRef }: { 
  value: string; 
  index: number; 
  ...
  return (
    <input
      ...
      onKeyDown={onKeyDown}
      className="w-[17.07%] h-full bg-[#f0f1f2] rounded-[8px] text-center font-['Poppins:Regular',sans-serif] text-[24px] text-[#37474f] border-none outline-none focus:ring-2 focus:ring-[#ffd037]"
      style={{ position: 'absolute', left: `${index * 20.73}%` }}
    />
  );
}

function Group7Copy({ otpValues, handleOTPChange, handleKeyDown, inputRefs }: { 
  otpValues: string[];
...
function FormElementsMobileOtpCodeActiveEmpty({ otpValues, handleOTPChange, handleKeyDown, inputRefs }: { 
  otpValues: string[];
  handleOTPChange: (index: number, value: string) => void;
  handleKeyDown: (index: number, e: KeyboardEvent<HTMLInputElement>) => void;
  inputRefs: React.RefObject<HTMLInputElement>[];
  ...
  return (
    <div className="absolute inset-[70.57%_6.4%_22.54%_6.13%]" data-name="form elements/mobile/OTP Code/active/empty">
      <Group7Copy otpValues={otpValues} handleOTPChange={handleOTPChange} handleKeyDown={handleKeyDown} inputRefs={inputRefs} />
    </div>
  );
}
...
export default function OTPScreen({ phoneNumber, onBack, onNext, onChangePhone, onResendOTP }: OTPScreenProps) {
  ...
  return (
    <div className="bg-white relative size-full" data-name="Splash--Phone Number--OTP">
      ...
      <div className="absolute font-['Poppins:Regular',sans-serif] inset-[60.59%_4.27%_32.88%_4.27%] leading-[normal] not-italic text-[0px] text-[rgba(55,71,79,0.72)]">
        ...
        <p className="font-['Poppins:Bold',sans-serif] text-[#37474f] text-[16px]">{phoneNumber}</p>
      </div>
      <ButtonsMobileSolidTextSymbolTextSymbol onClick={handleNext} disabled={!isComplete} />
      <button 
        onClick={onResendOTP}
        className="absolute font-['Poppins:Regular',sans-serif] inset-[81.28%_73.87%_16.13%_4.27%] leading-[normal] not-italic text-[#f37a20] text-[14px] text-nowrap whitespace-pre cursor-pointer bg-transparent border-none"
      >
        Resend OTP
      </button>
      <button 
        onClick={onChangePhone}
        className="absolute font-['Poppins:Regular',sans-serif] inset-[81.28%_6.13%_16.13%_50.13%] leading-[normal] not-italic text-[14px] text-[rgba(55,71,79,0.72)] text-nowrap text-right whitespace-pre cursor-pointer bg-transparent border-none"
      >
        Change Phone Number
      </button>
