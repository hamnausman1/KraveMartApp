...
import svgPaths from "../imports/svg-4pgoyq5rwy";
...
function Copy() {
  ...
  return (
    <div className="content-stretch flex flex-col gap-[2px] items-center leading-[1.5] not-italic relative shrink-0 text-black text-center text-nowrap whitespace-pre" data-name="Copy">
      <p className="font-['Inter:Semi_Bold',sans-serif] font-semibold relative shrink-0 text-[16px]">{t('createAccount')}</p>
      <p className="font-['Inter:Regular',sans-serif] font-normal relative shrink-0 text-[14px]">{t('enterEmail')}</p>
    </div>
  );
}
      ...
      <div className="flex flex-row items-center size-full">
        <div className="box-border content-stretch flex gap-[16px] h-[40px] items-center px-[16px] py-[8px] relative w-full">
          <input
            ...
            className="[white-space-collapse:collapse] basis-0 font-['Inter:Regular',sans-serif] font-normal grow leading-[1.4] min-h-px min-w-px not-italic overflow-ellipsis overflow-hidden relative shrink-0 text-[#828282] text-[14px] text-nowrap bg-transparent border-none outline-none placeholder:text-[#828282]"
          />
        </div>
      </div>
    </div>
...
function Button({ onClick, disabled }: { onClick: () => void; disabled?: boolean }) {
  ...
  return (
    <button 
      onClick={onClick}
      disabled={disabled}
      className="bg-black h-[40px] relative rounded-[8px] shrink-0 w-full disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed" 
      data-name="Button"
    >
      <div className="flex flex-row items-center justify-center size-full">
        <div className="box-border content-stretch flex gap-[8px] h-[40px] items-center justify-center px-[16px] py-0 relative w-full">
          <div className="flex flex-col font-['Inter:Medium',sans-serif] font-medium justify-center leading-[0] not-italic relative shrink-0 text-[14px] text-nowrap text-white">
            <p className="leading-[1.4] whitespace-pre">{t('continue')}</p>
          </div>
        </div>
      </div>
    </button>
...
function InputButton({ email, onEmailChange, onContinue }: { email: string; onEmailChange: (email: string) => void; onContinue: () => void }) {
  return (
    <div className="content-stretch flex flex-col gap-[16px] items-start relative shrink-0 w-[327px]" data-name="Input + Button">
      ...
      <Button onClick={onContinue} disabled={!email} />
    </div>
  );
}
...
function Divider() {
  ...
  return (
    <div className="content-stretch flex gap-[8px] items-center justify-center relative shrink-0 w-[327px]" data-name="Divider">
      ...
      <p className="font-['Inter:Regular',sans-serif] font-normal leading-[1.4] not-italic relative shrink-0 text-[#828282] text-[14px] text-center text-nowrap whitespace-pre">{t('or')}</p>
      <div className="basis-0 bg-[#e6e6e6] grow h-px min-h-px min-w-px shrink-0" data-name="Divider" />
    </div>
  );
}
  ...
  return (
    <div className="absolute content-stretch flex gap-[8px] items-center left-[calc(50%+0.5px)] top-1/2 translate-x-[-50%] translate-y-[-50%]" data-name="Label">
      ...
      <div className="flex flex-col font-['Inter:Medium',sans-serif] font-medium justify-center leading-[0] not-italic relative shrink-0 text-[14px] text-black text-nowrap">
        <p className="leading-[1.4] whitespace-pre">{t('continueWithGoogle')}</p>
      </div>
    </div>
  );
}
...
function Button1() {
  return (
    <button className="[grid-area:1_/_1] bg-[#eeeeee] h-[40px] ml-0 mt-0 relative rounded-[8px] w-[327px] cursor-pointer" data-name="Button">
      <Label />
    </button>
  );
}
...
function Label1() {
  ...
  return (
    <div className="absolute content-stretch flex gap-[8px] items-center left-[calc(50%+0.5px)] top-1/2 translate-x-[-50%] translate-y-[-50%]" data-name="Label">
      ...
      <div className="flex flex-col font-['Inter:Medium',sans-serif] font-medium justify-center leading-[0] not-italic relative shrink-0 text-[14px] text-black text-nowrap">
        <p className="leading-[1.4] whitespace-pre">{t('continueWithApple')}</p>
      </div>
    </div>
  );
}
...
function Button2() {
  return (
    <button className="[grid-area:1_/_1] bg-[#eeeeee] h-[40px] ml-0 mt-[48px] relative rounded-[8px] w-[327px] cursor-pointer" data-name="Button">
      <Label1 />
    </button>
  );
}
...
function Content({ email, onEmailChange, onContinue }: { email: string; onEmailChange: (email: string) => void; onContinue: () => void }) {
  const { t } = useLanguage();
  return (
    <div className="absolute box-border content-stretch flex flex-col gap-[24px] items-center left-[calc(50%+1px)] px-[24px] py-0 top-[calc(50%+22.5px)] translate-x-[-50%] translate-y-[-50%]" data-name="Content">
      <Copy />
      <InputButton email={email} onEmailChange={onEmailChange} onContinue={onContinue} />
      <Divider />
      <Buttons />
      <p className="font-['Inter:Regular',sans-serif] font-normal leading-[1.5] min-w-full not-italic relative shrink-0 text-[#828282] text-[0px] text-[12px] text-center w-[min-content]">
        <span>{t('byClickingContinue')} </span>
        <span className="text-black">{t('termsOfService')}</span>
        <span> {t('and')} </span>
        <span className="text-black">{t('privacyPolicy')}</span>
      </p>
    </div>
  );
}
...
export default function SignInScreen({ email, onEmailChange, onContinue, onLogin }: SignInScreenProps) {
  ...
  const handleSpeakerClick = () => {
    const text = language === 'english' 
      ? "This is the sign in screen. You can sign up, login, or change the language."
      : "یہ سائن ان اسکرین ہے۔ آپ سائن اپ کر سکتے ہیں، لاگ ان کر سکتے ہیں، یا زبان تبدیل کر سکتے ہیں۔";
    speak(text, language);
  };
  ...
  return (
    <div className="bg-[#ffd037] relative size-full" data-name="Sign In">
      <HomeIndicator />
      <Content email={email} onEmailChange={onEmailChange} onContinue={onContinue} />
      <p className="absolute font-['Inter:Regular',sans-serif] font-normal leading-[1.5] left-[188px] not-italic text-[14px] text-black text-center text-nowrap top-[717px] translate-x-[-50%] whitespace-pre">
        ...
        <button onClick={onLogin} className="text-[#007aff] cursor-pointer bg-transparent border-none underline">{t('login')}</button>
      </p>
      <button 
        onClick={toggleLanguage}
        className="absolute font-['Inter:Regular','Noto_Sans_Arabic:Regular',sans-serif] font-normal leading-[1.5] left-[187.5px] not-italic text-[#828282] text-[15px] text-center top-[652px] translate-x-[-50%] w-[327px] bg-transparent border-none cursor-pointer" 
        dir="auto"
      >
        <span className={language === 'english' ? 'text-black' : 'text-[#828282]'}>English</span>
        <span>{` | `}</span>
        <span className={language === 'urdu' ? 'text-black' : 'text-[#828282]'}>اردو</span>
      </button>
      <StatusBar />
      <Language />
      <button 
        ...
        aria-label="Read screen information"
      >
